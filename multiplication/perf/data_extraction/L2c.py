...
with open('L2_rate_cache.txt', 'r', encoding='utf-8') as file:  
    for line in file:  
        # 检查行是否包含所需的计数器  
...
# 将结果写入CSV文件  
with open('L2_rate_cache.csv', 'w', newline='', encoding='utf-8') as csvfile:  
    fieldnames = ['L2_rate_cache']  # 定义CSV文件的列名  
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)  
  
    writer.writeheader()  # 写入列名  
    for result in results:  
        writer.writerow({'L2_rate_cache': result})  # 写入每一行的结果  
  
print("结果已写入L2_rate_cache.csv文件。")