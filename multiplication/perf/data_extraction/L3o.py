...
with open('L3_rate_ordinary.txt', 'r', encoding='utf-8') as file:  
    for line in file:  
        # 检查行是否包含所需的计数器  
...
print(results)  
# 将结果写入CSV文件  
with open('L3_rate_ordinary.csv', 'w', newline='', encoding='utf-8') as csvfile:  
    fieldnames = ['L3_rate_ordinary']  # 定义CSV文件的列名  
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)  
  
    writer.writeheader()  # 写入列名  
    for result in results:  
        writer.writerow({'L3_rate_ordinary': result})  # 写入每一行的结果  
print(len(A))
print("结果已写入L3_rate_ordinary.csv文件。")