...
bool* C;
bool* D;
int num,row,col1,col2,comrow,nowcol,nowrow,Time;
ifstream In;
ofstream Out;

const int MPI_US=sizeof(__uint128_t);
//unsigned short* SSA;
//unsigned short* SA;
//unsigned short** SB;
//unsigned short* SSB;
...
void subprocess_read()
{
	...
	SSB=new __uint128_t[Num*comrow];
	SSA=new __uint128_t[comrow];
	SB=new __uint128_t*[Num];
	SD=new bool[Num];
	for(int i=0;i<Num;i++)
	{
		SB[i]=&SSB[i*comrow];
...
void read()
{
	...
	//声明全长的unsigned short数组，再用unsigned short*数值切割为二维
	BB=new __uint128_t[col2*comrow];
...
bool check()
{
	for(int i=0;i<col2;i++)
	{
		...
		{
			if((A[nowrow][j>>offset]&Bit[j&Maxn])!=0) return 0;
		}
		if( (A[nowrow][nowrow>>offset]&Bit[nowrow&Maxn]) ==0) return 0;
		for(int j=nowrow+1;j<row;j++)
		{
			if( (A[nowrow][j>>offset]&Bit[j&Maxn]) !=0)
			{
				In>>nowcol;
...
//---------------------------------------------------------------------------------	
	...
	for(int i=1;i<=9;i++)
	{
		...
		if(world_rank==0) 
		{
			cout<<i<<endl;
			Out<<"test"<<i<<",";
			In.open("/home/s2213917/Composite/tests/test"+to_string(i)+".txt");
			In>>row>>col1>>col2;
		}
...
//---------------------------------前向消元----------------------------------------
		//cerr<<"进程"<<world_rank<<"即将进入前向消元"<<endl;
...
//---------------------------------记录时间----------------------------------------		
		if(world_rank==0)
		{
			...
			/*for(int i=0;i<row;i++)
			{
				...
				for(int j=0;j<row;j++)
				{
					if( (A[i][j>>offset]&Bit[j&Maxn]) !=0) cout<<j<<" ";
				}
				cout<<endl;
			}*/
//---------------------------------验证---------------------------------------------
			if(!In) cerr<<i<<"打开文件失败"<<endl;
			if(check()==0) cerr<<i<<"error!"<<endl;
